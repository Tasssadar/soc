...
\begin{itemize}
    \item {\bf SerialChart}\footnote{\It{Analyse and chart serial data from RS-232 COM ports} -- \url{http://code.google.com/p/serialchart/}} je open-source program\footnote{Program s otevřeným zdrojovým kódem} pro parsování a zobrazování dat přicházející ze sériového portu. Je jednoduchý a přehledný, dokáže však zobrazovat pouze graf a nastavení je třeba ručně napsat.
    \item {\bf WinWedge}\footnote{\It{RS232 data collection software} -- \url{http://www.taltech.com/products/winwedge/}} je komerční program který dokáže zpracovávat data přicházející sériovým portem a zobrazovat je jako graf v MS Excel nebo ve webové stránce. Dokáže také posílat příkazy zpět do zařízení, oproti Lorris má však horší ovládání (hlavně kvůli nutnosti použít další program pro zobrazování), má užší možnosti použití a je dostupný pouze pro MS Windows.
    \item {\bf StampPlot Pro}\footnote{\It{Graphical Data Acquisition and Control} -- \url{http://www.selmaware.com/stampplot/index.htm}} dokáže zobrazovat příchozí data ve widgetech zvolených uživatelem, podobně jako modul Analyzér v Lorris, má však komplikované ovládání, nemá otevřený zdrojový kód, je dostupný pouze pro MS Windows a pod MS Windows 7 nefunguje.
\end{itemize}

\section*{Popis rozhraní}
\addcontentsline{toc}{section}{Popis rozhraní}
Svůj program jsem pojmenoval "Lorris", je vytvořený v C++ a využívá Qt Framework\footnote{\It{Cross--platform application and UI framework} -- \url{http://qt.nokia.com/}} (v4.7), což je multiplatformní framework, který mimo jiné umožňuje spustit aplikaci na více systémech -- testoval jsem na Debian Linux\footnote{\It{The Universal Operating System} -- \url{http://www.debian.org/}} (Wheezy, 64bit) a Windows 7.  

\subsection*{Web a repozitář programu}
\addcontentsline{toc}{subsection}{Web a repozitář programu}
GIT\footnote{\It{GIT} -- distribuovaný systém správy verzí} repozitář programu jsem vytvořil na serveru GitHub\footnote{\It{GitHub -- Social Coding} -- \url{https://github.com}}, který kromě hostingu repozitáře poskytuje i několik dalších služeb, mezi nimi i hosting webu projektu. Na webu který jsem vytvořil jsou odkazy ke stažení předkompilovaných souborů pro Windows, popis programu, krátké video s představením programu, screenshoty a návod ke zkompilování.

\begin{itemize}
    \item Repozitář: \url{https://github.com/Tasssadar/Lorris}
    \item Web: \url{http://tasssadar.github.com/Lorris/cz/index.html}
\end{itemize}
...
Struktura dat se nastavuje v samostatném dialogu (viz obrázek 3), kde je možno nastavit délku packetu, jeho endianness\footnote{\It{Endianness} -- pořadí uložení bytů v paměti počítače}, přitomnost hlavičky a její obsah -- statická data ("start byte"), délka packetu(pokud je proměnná), příkaz a ID zařízení. Podle příkazu a ID zařízení je možno později data filtrovat.
...
Shupito je programátor mikročipů vytvořený Martinem Vejnárem, který dokáže programovat mikrokontroléry pomocí ISP\footnote{\It{In-system programming} -- rozhraní, které umožňuje programovat čipy bez dalšího zařízení přímo v desce plošného spoje.}, PDI\footnote{\It{Program and Debug Interface} -- rohraní firmy Atmel umožňující programování čipů přímo na desce, podobně jako ISP} a JTAG\footnote{\It{Joint Test Action Group} -- rozhraní podle standartu IEEE 1149.1 umožňující mimo jiné programování a debugování čipů} rozhraní. 
...
\newpage
\subsection*{4. Vývoj robota pro soutěž Eurobot 2011}
...
Robot našeho týmu byl poměrně jednoduchý, přesto však obsahoval 5 ultrazvukových měřáků vzdálenosti, dva enkodéry a 5 tlačítek (detekce nárazu na mantinel a pěšce, který mohl robot převážet). Tyto senzory produkují poměrně značné množství dat, které se v terminálu zobrazuje nepřehledně.
...
Toto byl také jeden z důvodů pro výběr toho tématu práce. S použitím programu Lorris vypádá výstup následnovně:
\begin{figure}[H]
\begin{center}
...
Widget "Robot" uprostřed reprezentuje tělo robota. Widgety "číslo" s názvy "Ultra E0...8" ukazují vzdálenost z ultrazvukových měřáků. Ve widgetu "Ultra E0" je vzdálenost menší než 25 cm, což je hraniční vzdálenost po které se robot zastaví aby nevrazil do soupeře - aby na tuto skutečnost upozornil, má červené pozadí.
...
Poslední widgety "Enc left" a "Enc right" vypisují ujetou vzdálenost z enkodérů pravého a levého kola.

Všechny widgety jsou napozicované stejně jako na robotovi - 2 ultrazvuky měří vzdálenost vepředu, 3 vzadu; tlačítka pro detekci mantinelu jsou na každém rohu atd. 
\newpage
...
{\bf Qwt}\footnote{\It{Qt Widgets for Technical Applications} -- \url{http://qwt.sourceforge.net/}} je knihovna pro Qt Framework obsahující tzv. widgety pro aplikace technického charakteru -- grafy, sloucové ukazatele, kompasy a podobně. Ve svojí práci
zatím z této knihovny používám pouze graf (v modulu analyzéru). \\
{\bf QExtSerialPort}\footnote{\It{Qt interface class for old fashioned serial ports} -- \url{http://code.google.com/p/qextserialport/}} poskytuje připojení k sériovému portu a také dokáže vypsat seznam nalezených portů v počítačí.\\
{\bf QHexEdit2}\footnote{\It{Binary Editor for Qt} -- \url{http://code.google.com/p/qhexedit2/}} je hex editor použitý v modulu programátoru Shupito na zobrazování obsahu paměti. V této knihovně jsem upravoval několik málo drobností, 
týkajících se především vzhledu.

\newpage
\subsection*{Licence}
\addcontentsline{toc}{subsection}{Licence}
Lorris je dostupný pod licencí GNU GPLv3\footnote{\It{GNU General Public License} -- \url{http://gplv3.fsf.org/}}, licence použitých programů a knihoven jsou následující:
\begin{itemize}
    \item {\bf Qt Framework} je distribuován pod licencí GNU LGPLv2.1\footnote{\It{GNU Lesser General Public License} -- \url{http://www.gnu.org/licenses/lgpl-2.1.html}}
    \item {\bf Qwt} je distribuováno pod Qwt license\footnote{\It{Qwt license} -- \url{http://qwt.sourceforge.net/qwtlicense.html}}, která je založená na GNU LGPLv2.1
    \item {\bf QExtSerialPort} je distribuován pod The New BSD License\footnote{\It{The New BSD License} -- \url{http://www.opensource.org/licenses/bsd-license.php}}
    \item {\bf QHexEdit2} je distribuován pod licencí GNU LGPLv2.1
    \item {\bf avr232client} je distribuován pod licencí Boost Software License v1.0\footnote{\It{Boost Software License} -- \url{http://www.boost.org/users/license.html}}
\end{itemize}
